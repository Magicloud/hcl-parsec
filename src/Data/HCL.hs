...
hclBool = label "hclBool" $ do
  s <- getSourcePos
  b <- strTrue <|> strFalse
  e <- getSourcePos
  return $ VBool s e b
  where
    strTrue = hclSymbol "true" <|> hclSymbol "TRUE" <|> hclSymbol "True" >> return True
    strFalse = hclSymbol "false" <|> hclSymbol "FALSE" <|> hclSymbol "False" >> return False

hclString :: Parser Value
hclString = label "hclString" $ do
  s <- getSourcePos
  void $ char '"'
...
hclStringPart = try hclStringInterpEscape <|> hclStringInterp <|> hclStringPlain

hclStringInterpEscape :: Parser StringPart
hclStringInterpEscape = label "hclStringInterpEscape" $ do
  s <- getSourcePos
  void $ string "$${"
  e <- getSourcePos
...
hclStringInterp = label "hclStringInterp" $ do
  s <- getSourcePos
  void $ symbol (void hclSkipSpace) "${"
  str <- takeWhile1P Nothing (/= '}')
  e <- getSourcePos
  return $ SPInterp $ PT s e str

hclStringPlain :: Parser StringPart
hclStringPlain = label "hclStringPlain" $ do
  let end = eof
        <|> lookAhead (void $ char '"')
...
-- TODO: Interpolation in multi-line string
hclStringPlainML :: Parser Value
hclStringPlainML = label "hclStringPlainML" $ do
  s <- getSourcePos
  void $ string "<<"
  indent <- optional $ char '-'
  eof' <- takeWhile1P Nothing (\c -> isAlphaNum c || c `elem` ['_', '-', '.'])
  void eol
  t <- Text.intercalate "\n" <$> manyTill line (when (isJust indent) hclSkipSpace' >> string eof')
  e <- getSourcePos
  return $ VString s e [SPPlain $ PT s e t]

line :: Parser Text
line = Text.pack <$> someTill charLiteral eol
...
hclSkipLineComment = label "hclSkipLineComment" $ skipLineComment "#" <|> skipLineComment "//"
...
hclIndexing = label "hclIndexing" $ do
  s <- getSourcePos
  array <- hclLexeme $ try hclFunction <|> hclIdents <|> hclList <|> hclMap
  void $ hclSymbol "["
...
hclValue = label "hclValue" $ choice
  [ hclNumber
  , try $ choice [ try hclFunction
                 , hclIdents
                 , hclList
                 , hclMap] <* notFollowedBy (char '[')
  , hclIndexing
  , hclString
  , hclBool
  , hclObject ]
...
hclFunction = label "hclFunction" $ do
  s <- getSourcePos
  fn <- hclIdent
  void $ hclSymbol "("
...
hclPair = label "hclPair" $ do
  s <- getSourcePos
  k <- hclLexeme_ $ doubleQuotedString <|> hclIdent
  void $ hclSymbol "="
  v <- hclLexeme_ $ hclStringPlainML <|> hclValue
  e <- getSourcePos
  return $ VPair s e k v
  where
    hclLexeme_ = L.lexeme hclSkipSpace_
...
doubleQuotedString = label "doubleQuotedString" $ do
  s <- getSourcePos
  void $ char '"'
...
hclObject = label "hclObject" $ do
  s <- getSourcePos
  is <- hclLexeme $ manyTill (hclLexeme $ doubleQuotedString <|> hclIdent) $ char '{'
  vs <- hclLexeme $ manyTill (hclLexeme $ try hclPair <|> hclObject) $ char '}'
  let (os_, as_) = foldr (\i (os, as) -> case i of
                        o@VObject{} -> (o : os, as)
                        a@VPair{} -> (os, a : as)
                        _ -> (os, as)
                      ) ([], []) vs
