               ...
               -> foldl (\tfByLine (MR (PT s e t) v)
                         -> let tfByLine' =
                                  case v of
                                    ...
                                    Just (PT sv ev _)
                                      -> let ( pre, _, post ) =
                                               extractRange tfByLine
                                                            (unPos
                                                             $ sourceLine sv)
                                                            (unPos
                                                             $ sourceLine ev)
                                         in pre ++ post
                            in replaceAtWith tfByLine' s e
                               $ reg2src t base ver_)
                        (T.lines tf)
                        modSrcVers
             _ -> undefined
...
modSV (VObject _ _ (PT _ _ "module":_) ps _) =
  Just
  $ foldl (\mr (VPair _ _ (PT _ _ k) v) -> case k of
             "source" -> let (VString _ _ [ SPPlain pt ]) = v
                         in mr { source = pt }
             "version" -> let (VString _ _ [ SPPlain pt ]) = v
                          in mr { version = Just pt }
             _ -> mr)
          (MR (PT (initialPos "") (initialPos "") "") Nothing)
          ps
modSV _ = Nothing
...
src2reg src baseUrl =
  ...
  in T.intercalate "/"
                   ([ baseUrl, orgName, platform ]
                    ++ init modPath
                    ++ [ T.takeWhile (/= '?') (last modPath) ])
...
reg2src reg baseUrl ref =
  ...
  in T.intercalate "/"
                   ([ baseUrl
                    , T.intercalate "-" [ "terraform", platform, orgName ]
                        `T.append` ".git" ]
                    ++ drop (idxOfMod - 1) parts)
     `T.append` maybe "" ("?ref=" `T.append`) ref
